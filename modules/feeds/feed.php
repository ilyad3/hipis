...
if ($session_key == NULL) {
	...
	if ($user_row == 0) {
		...
		if ($vk_key == NULL AND $fb_key == NULL AND $inst_key == NULL AND $twit_key == NULL) {
			header("Location: http://hipis.ru/aa");
		}else{
			if ($vk_key != NULL) {
				$vk_query = file_get_contents("https://api.vk.com/method/newsfeed.get?access_token=".$vk_key."&filters=post,photo&count=".$count_news."&v=5.34");
...
$menu = "<li><a href='/feed'><img src='img/menu_ico/news_active_ico.png'></a></li>
			<li><a href='/setting'><img src='img/menu_ico/setting_ico.png'></a></li>
			<li><a href='/posts'><img src='img/menu_ico/posts_ico.png'></a></li>";
$smarty->assign("menu", $menu);
$smarty->assign("title", "Новостная лента");
$body = "<div class='wall'>".$body."</div><div id='js'><script language='javascript'>var scroll_id = '0'</script></div>";
